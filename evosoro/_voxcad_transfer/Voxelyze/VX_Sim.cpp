...
double CVX_Sim::getAverageStiffnessChange()
{
	...
	for (int i=0; i<iT; i++) {
	    double this_change = VoxArray[i].getCurStiffnessChange()/iT;
	    if (this_change > 0) {
	        averageStiffnessChange += this_change;
	    }
	    else {
	        averageStiffnessChange -= this_change;
	    }
...
std::vector< vfloat > CVX_Sim::getVoxelStiffnessChanges()
{
    std::vector< vfloat > StiffnessChange;
    int iT = NumVox();
	for (int i=0; i<iT; i++) {
	    vfloat this_change = VoxArray[i].getCurStiffnessChange();
	    StiffnessChange.push_back(this_change);
    }

	return StiffnessChange;

}


std::vector< Vec3D<> > CVX_Sim::getVoxelPositions()
{
    std::vector< Vec3D<> > Coordinates;
    int iT = NumVox();
	for (int i=0; i<iT; i++) {
	    Coordinates.push_back(VoxArray[i].GetNominalPosition());
    }
	return Coordinates;
}


double CVX_Sim::getVarianceStiffnessChange()
{

	int iT = NumVox();
	double Ex2 = 0.0;
	double mu = getAverageStiffnessChange();

	for (int i=0; i<iT; i++) {
	    double this_change = VoxArray[i].getCurStiffnessChange();
	    Ex2 += this_change*this_change/iT;
    }
...
double CVX_Sim::getSkewnessStiffnessChange()
{

	int iT = NumVox();
	double mu = getAverageStiffnessChange();
	double sigmaSquared = getVarianceStiffnessChange();
	double Skewness = 0.0;

	for (int i=0; i<iT; i++) {
	    double this_change = VoxArray[i].getCurStiffnessChange();
	    double moment = (this_change-mu)*(this_change-mu)*(this_change-mu)/iT;
	    if (this_change > 0) {
	        Skewness += moment;
	    }
	    else {
	...
	return Skewness / pow(sigmaSquared, 3.0/2.0);

}


double CVX_Sim::getKurtosisStiffnessChange()
{
	...
	for (int i=0; i<iT; i++) {
	    double this_change = VoxArray[i].getCurStiffnessChange();
	    double moment = (this_change-mu)*(this_change-mu)*(this_change-mu)*(this_change-mu)/iT;
	    if (this_change > 0) {
	        Kurt += moment;
	    }
	    else {
	...
	return Kurt / (sigmaSquared*sigmaSquared);
