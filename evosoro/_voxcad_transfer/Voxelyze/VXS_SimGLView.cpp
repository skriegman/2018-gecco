...
CColor CVXS_SimGLView::GetCurVoxColor(int SIndex, int Selected)
{
	...
	//			LocalVXC.GetLeafMat(VoxArray[SIndex].GetVxcIndex())->GetColorf(&R, &G, &B, &A);
			...
			break;
		case RVC_KINETIC_EN: // Quickly remapped on delta stiffness
		...
		case RVC_DISP: // WAS: displacement. Quickly remapped onto stress adaptation rate
		{
		...
		case RVC_STRAIN_EN: // WAS: strain energy. Quickly remapped onto pressure adaptation rate
		{
