...
// 	// std::cout << "totalPoints: " << totalPoints << std::endl;
	...
	float volStart = internalMesh->getRobotVolumeStart();
	float qhullStart = internalMesh->getRobotQhullVolumeStart();
	float volEnd = internalMesh->getRobotVolumeEnd();
	float qhullEnd = internalMesh->getRobotQhullVolumeEnd(); 
	float shapeComplexityStart = internalMesh->getShapeComplexityStart();
	float shapeComplexityEnd = internalMesh->getShapeComplexityEnd();
	int numVoxels = NumVox();
	float avgStiffnessChange = getAverageStiffnessChange();
	float varStiffnessChange = getVarianceStiffnessChange();
	float skewStiffnessChange = getSkewnessStiffnessChange();
	float kurtStiffnessChange = getKurtosisStiffnessChange();
	std::vector< vfloat > VoxelStiffnessChanges = getVoxelStiffnessChanges();
	std::vector< Vec3D<> > VoxelPositions = getVoxelPositions();
	...
	if(simToCombine) // This is used when two SimGA objects are passed to this function, and results should be combined (same individual evaluated in two different environments)
	{
		...
		volStart   			  = (volStart   +  internalMesh2->getRobotVolumeStart())/2;
		qhullStart 			  = (qhullStart +  internalMesh2->getRobotQhullVolumeStart())/2;
		volEnd 	   			  = (volEnd 	+  internalMesh2->getRobotVolumeEnd())/2;
		qhullEnd   			  = (qhullEnd   +  internalMesh2->getRobotQhullVolumeEnd())/2; 
		numVoxels 			  = (numVoxels	+  simToCombine->NumVox())/2;		
		shapeComplexityStart  = (shapeComplexityStart + internalMesh2->getShapeComplexityStart())/2;
		shapeComplexityEnd 	  = (shapeComplexityEnd   + internalMesh2->getShapeComplexityEnd())/2;
	...
	pXML->Element("VarianceStiffnessChange", varStiffnessChange);
	pXML->Element("SkewnessStiffnessChange", skewStiffnessChange);
	pXML->Element("KurtosisStiffnessChange", kurtStiffnessChange);

	pXML->DownLevel("StiffnessChange");
        for(std::vector<vfloat>::size_type i = 0; i != VoxelStiffnessChanges.size(); ++i)
        {
            pXML->DownLevel("Voxel");
                pXML->Element("Gain", VoxelStiffnessChanges[i]);
                pXML->Element("X", VoxelPositions[i].x);
                pXML->Element("Y", VoxelPositions[i].y);
                pXML->Element("Z", VoxelPositions[i].z);
            pXML->UpLevel();
        }
    pXML->UpLevel();
	...
	pXML->Element("RobotVolumeStart", volStart);
	pXML->Element("ConvexHullVolumeStart", qhullStart);
	pXML->Element("RobotVolumeEnd", volEnd);
	pXML->Element("ConvexHullVolumeEnd", qhullEnd);	
	pXML->Element("ShapeComplexityStart", shapeComplexityStart);
	pXML->Element("ShapeComplexityEnd", shapeComplexityEnd);
