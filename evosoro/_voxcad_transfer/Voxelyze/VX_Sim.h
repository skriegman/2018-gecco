...
//	void SetStopCondition(StopCondition StopConditionTypeIn = SC_NONE, vfloat StopConditionValueIn = 0.0) {SetStopConditionType(StopConditionTypeIn); SetStopConditionValue(StopConditionValueIn);}
    ...
    // get change in stiffness for each voxel
    std::vector< vfloat > getVoxelStiffnessChanges();
    std::vector< Vec3D<> > getVoxelPositions();
...
//	vfloat DragConstant;
	...
	double getVarianceStiffnessChange();
	double getSkewnessStiffnessChange();
	double getKurtosisStiffnessChange();
